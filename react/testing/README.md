---
description: Общие принципы написания тестов React-компонентов.
---
...
* [Подготовка/Завершение](https://ru.reactjs.org/docs/testing-recipes.html#setup--teardown)
* [`act()`](https://ru.reactjs.org/docs/testing-recipes.html#act)
* [Рендеринг](https://ru.reactjs.org/docs/testing-recipes.html#rendering)
* [Получение данных](https://ru.reactjs.org/docs/testing-recipes.html#data-fetching)
* [Фиктивные модули](https://ru.reactjs.org/docs/testing-recipes.html#mocking-modules)
* [События](https://ru.reactjs.org/docs/testing-recipes.html#events)
* [Таймеры](https://ru.reactjs.org/docs/testing-recipes.html#timers)
* [Тестирование снимками](https://ru.reactjs.org/docs/testing-recipes.html#snapshot-testing)
* [Несколько рендереров](https://ru.reactjs.org/docs/testing-recipes.html#multiple-renderers)
* [Чего-то не хватает?](https://ru.reactjs.org/docs/testing-recipes.html#something-missing)

#### Подготовка/Завершение <a id="setup--teardown"></a>
...
#### Получение данных <a id="data-fetching"></a>

Вместо обращения к реальным API в своих тестах, вы можете делать фиктивные запросы, которые вернут подставные данные. Такие запросы предотвращают проблемы в тестах, связанные с недоступностью бэкенда, и увеличивают скорость их выполнения. Примечание: вы всё ещё можете запускать набор [«сквозных»](https://ru.reactjs.org/docs/testing-environments.html#end-to-end-tests-aka-e2e-tests) тестов через фреймворк, которые проверяют, как работает приложение в целом.
...
#### Фиктивные модули <a id="mocking-modules"></a>
...
{% tab title="contact.test.js" %}
```javascript
// Если мы не хотим загружать этот компонент в наш тест, 
// мы можем подменить его фиктивным и запустить наши тесты
...
beforeEach(() => {
  // подготавливаем DOM-элемент, куда будем рендерить
  container = document.createElement("div");
  // container *обязан* быть прикреплен к document, чтобы события работали правильно.  document.body.appendChild(container);});
...
{% tab title="card.test.js" %}
```javascript
// card.test.js

// Давайте напишем тесты для этого компонента, используя 
...
#### Тестирование снимками <a id="snapshot-testing"></a>
...
В большинстве случаев лучше проверять специфические вероятные результаты, чем использовать снимки. Этот тип тестов опирается на внутреннюю реализацию компонентов, в результате тесты легко ломаются и команды начинают уделять меньше внимания поломкам в снимках. Выборочная [подмена дочерних компонентов](https://ru.reactjs.org/docs/testing-recipes.html#mocking-modules) поможет снизить размер снимков и сделает их код более читаемым для коллег.

#### Несколько рендереров <a id="multiple-renderers"></a>
...
Source: [https://ru.reactjs.org/docs/testing-recipes.html](https://ru.reactjs.org/docs/testing-recipes.html)
